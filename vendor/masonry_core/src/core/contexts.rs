...
impl MutateCtx<'_> {
    /// Transfer text focus while mutating a widget from a view rebuild.
...
impl UpdateCtx<'_> {
    /// Transfer text focus during an update pass.
    ///
    /// This is primarily useful for widgets that are inserted in response to an action and
    /// should immediately become the active text editor.
    pub fn set_focus(&mut self, target: WidgetId) {
        trace!("set_focus target={:?}", target);
        self.global_state.next_focused_widget = Some(target);
    }
...
impl_context_method!(
    ...
    RawCtx<'_>,
    {
        ...
        /// Synchronize the complete editable text state with the platform IME.
