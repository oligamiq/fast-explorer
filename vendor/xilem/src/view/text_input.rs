...
impl<State: 'static, Action: 'static> TextInput<State, Action> {
    ...
    /// Set the font stack used by this text input.
    ...
    /// Give this input focus as soon as it is inserted.
    ...
    pub fn initial_selection(mut self, start: usize, end: usize) -> Self {
...
impl<State: 'static, Action: 'static> View<State, Action, ViewCtx> for TextInput<State, Action> {
    ...
    fn build(&self, ctx: &mut ViewCtx, _: &mut State) -> (Self::Element, Self::ViewState) {
        ...
        let text_area = widgets::TextArea::new_editable(&self.contents)
            // TextInput is a single-line control. InsertNewline controls whether Enter
            // mutates/submits the value, but visual wrapping must never turn an address,
            // search query, or filename into multiple rows on a narrow viewport.
