...
impl RenderContext {
    ...
    pub(crate) async fn create_render_surface<'w>(
        ...
        // Android may report a transient 0×0 native surface while the Activity is
        // being attached/resumed. wgpu textures and surface configurations require
        // non-zero dimensions, so keep the renderer alive until the real resize arrives.
    ...
    async fn new_device(&mut self, compatible_surface: Option<&Surface<'_>>) -> Option<usize> {
        let force_cpu = std::env::var("FASTEXPLORER_FORCE_CPU")
            .ok()
            .is_some_and(|value| {
                matches!(
        ...
        if force_cpu {
            let adapter = self.instance.request_adapter(&fallback_options).await.ok()?;
            eprintln!(
                "FastExplorer: forced CPU/software renderer: {}",
                adapter.get_info().name
            );
            let handle = Self::request_device_handle(adapter).await?;
            self.devices.push(handle);
            return Some(self.devices.len() - 1);
        }

        match wgpu::util::initialize_adapter_from_env_or_default(&self.instance, compatible_surface)
            .await
        {
            Ok(adapter) => {
                ...
                if info.device_type == wgpu::DeviceType::Cpu {
                    eprintln!(
                        "FastExplorer: wgpu automatically selected CPU/software renderer: {}",
                        info.name
                    );
                }
                if let Some(handle) = Self::request_device_handle(adapter).await {
                    self.devices.push(handle);
                    return Some(self.devices.len() - 1);
                }
                eprintln!(
                    "FastExplorer: selected renderer could not create a device; trying CPU/software fallback"
                );
            }
            Err(error) => eprintln!(
                "FastExplorer: hardware renderer unavailable ({error}); trying CPU/software fallback"
            ),
        }

        let adapter = self.instance.request_adapter(&fallback_options).await.ok()?;
        eprintln!(
            "FastExplorer: using CPU/software renderer: {}",
            adapter.get_info().name
        );
        let handle = Self::request_device_handle(adapter).await?;
        self.devices.push(handle);
    ...
    async fn request_device_handle(adapter: wgpu::Adapter) -> Option<DeviceHandle> {
