...
impl MasonryState<'_> {
    ...
    fn render(
        ...
        {
            let _render_poll_span = tracing::info_span!("Polling GPU after rendering").entered();
    ...
    fn handle_signals(&mut self, event_loop: &ActiveEventLoop, app_driver: &mut dyn AppDriver) {
        ...
        loop {
            ...
            match signal {
                ...
                RenderRootSignal::ImeTextState { text, selection, compose } => {
                    #[cfg(target_os = "android")]
                    handle.set_ime_buffer_state(text, selection, compose);
                    #[cfg(not(target_os = "android"))]
                    let _ = (text, selection, compose);
                }
