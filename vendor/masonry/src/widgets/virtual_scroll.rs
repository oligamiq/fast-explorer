...
use crate::theme;
use crate::util::{debug_panic, fill_color, stroke};
...
pub struct VirtualScroll {
    ...
    /// Active touch gesture used to scroll on touch-first platforms.
    /// Stores pointer id, starting pointer Y, and the estimated absolute scroll position.
    touch_pan: Option<(PointerId, f64, f64)>,
    /// Anchor within the scrollbar thumb while it is being dragged, from 0.0 to 1.0.
    scrollbar_drag_anchor: Option<f64>,
...
impl std::fmt::Debug for VirtualScroll {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("VirtualScroll")
            ...
            .field("touch_pan", &self.touch_pan)
            .field("scrollbar_drag_anchor", &self.scrollbar_drag_anchor)
...
// A full-height touch drag is scaled to the estimated scrollable range. These bounds keep
// extremely short and extremely large virtual lists from producing unusably small or explosive
// motion while preserving exact end-to-end travel for normal file-list sizes.
const TOUCH_SCROLL_GAIN_MIN: f64 = 1.0;
const TOUCH_SCROLL_GAIN_MAX: f64 = 2.0;
const TOUCH_EDGE_SNAP_FRACTION: f64 = 0.90;

impl VirtualScroll {
    fn finite_item_count(&self) -> Option<f64> {
        if self.valid_range.start == i64::MIN || self.valid_range.end == i64::MAX {
            return None;
        }
        Some((self.valid_range.end - self.valid_range.start).max(0) as f64)
    }

    fn estimated_content_height(&self) -> Option<f64> {
        self.finite_item_count()
            .map(|count| count * self.mean_item_height.max(0.01))
    }

    fn estimated_max_scroll_y(&self, viewport_height: f64) -> Option<f64> {
        self.estimated_content_height()
            .map(|height| (height - viewport_height.max(0.0)).max(0.0))
    }

    fn estimated_scroll_y(&self, viewport_height: f64) -> Option<f64> {
        let max_scroll = self.estimated_max_scroll_y(viewport_height)?;
        let index_offset = (self.anchor_index - self.valid_range.start).max(0) as f64;
        Some(
            (index_offset * self.mean_item_height + self.scroll_offset_from_anchor)
                .clamp(0.0, max_scroll),
        )
    }

    fn touch_scroll_gain(&self, viewport_height: f64) -> Option<f64> {
        let max_scroll = self.estimated_max_scroll_y(viewport_height)?;
        Some(
            (max_scroll / viewport_height.max(1.0))
                .clamp(TOUCH_SCROLL_GAIN_MIN, TOUCH_SCROLL_GAIN_MAX),
        )
    }

    fn touch_scroll_target(
        &self,
        start_scroll: f64,
        finger_delta: f64,
        viewport_height: f64,
        ...
        let max_scroll = self.estimated_max_scroll_y(viewport_height)?;
        if finger_delta >= viewport_height * TOUCH_EDGE_SNAP_FRACTION {
            return Some(max_scroll);
        }
        if finger_delta <= -viewport_height * TOUCH_EDGE_SNAP_FRACTION {
            return Some(0.0);
        }
        let gain = self.touch_scroll_gain(viewport_height).unwrap_or(1.0);
        Some((start_scroll + finger_delta * gain).clamp(0.0, max_scroll))
    }

    fn set_estimated_scroll_y(
        &mut self,
        target: f64,
        viewport_height: f64,
    ) -> Option<PostScrollResult> {
        let max_scroll = self.estimated_max_scroll_y(viewport_height)?;
        if self.valid_range.is_empty() {
            return None;
        }
        let target = target.clamp(0.0, max_scroll);
        let item_height = self.mean_item_height.max(0.01);
        let index_offset = (target / item_height).floor() as i64;
        let next_anchor = (self.valid_range.start + index_offset)
            .clamp(self.valid_range.start, self.valid_range.end - 1);
        let next_offset = target - ((next_anchor - self.valid_range.start) as f64 * item_height);
        let anchor_changed = self.anchor_index != next_anchor;
        if !anchor_changed && (self.scroll_offset_from_anchor - next_offset).abs() <= f64::EPSILON {
            return None;
        }
        self.anchor_index = next_anchor;
        self.scroll_offset_from_anchor = next_offset.max(0.0);
        Some(if anchor_changed {
            PostScrollResult::Layout
        } else {
            PostScrollResult::NoLayout
        })
    ...
    fn scrollbar_geometry(&self, size: Size) -> Option<(Rect, Rect, f64)> {
        let content_height = self.estimated_content_height()?;
        let max_scroll = (content_height - size.height).max(0.0);
        if max_scroll <= 0.0 || size.height <= 0.0 || size.width <= 0.0 {
            return None;
        }
        let track_width = theme::SCROLLBAR_WIDTH + theme::SCROLLBAR_PAD * 2.0;
        let track = Rect::new((size.width - track_width).max(0.0), 0.0, size.width, size.height);
        let thumb_height = ((size.height / content_height) * size.height)
            .clamp(theme::SCROLLBAR_MIN_SIZE.min(size.height), size.height);
        let travel = (size.height - thumb_height).max(0.0);
        let progress = (self.estimated_scroll_y(size.height).unwrap_or(0.0) / max_scroll)
            .clamp(0.0, 1.0);
        let thumb_y = progress * travel;
        let thumb = Rect::new(
            track.x0 + theme::SCROLLBAR_PAD,
            thumb_y + theme::SCROLLBAR_PAD,
            track.x1 - theme::SCROLLBAR_PAD,
            (thumb_y + thumb_height - theme::SCROLLBAR_PAD).max(thumb_y + theme::SCROLLBAR_PAD),
        );
        Some((track, thumb, max_scroll))
    }

    fn request_estimated_scroll_update(ctx: &mut EventCtx<'_>, result: PostScrollResult) {
        match result {
    ...
    fn drag_scrollbar_to(
        ...
        let (_, thumb, max_scroll) = self.scrollbar_geometry(size)?;
        let thumb_height = thumb.height() + theme::SCROLLBAR_PAD * 2.0;
        let travel = (size.height - thumb_height).max(0.0);
        if travel <= 0.0 {
            return None;
        }
        let progress = ((pointer_y - grab_anchor * thumb_height) / travel).clamp(0.0, 1.0);
        self.set_estimated_scroll_y(progress * max_scroll, size.height)
    }
}
...
impl Widget for VirtualScroll {
        ...
        match event {
            PointerEvent::Down(PointerButtonEvent { pointer, state, .. }) => {
                    ...
                    && track.contains(local)
                {
                    ...
                    let grab_anchor = if thumb.contains(local) {
                        ((local.y - full_thumb_y0) / full_thumb_height).clamp(0.0, 1.0)
                    } else {
                        0.5
                    };
                    self.scrollbar_drag_anchor = Some(grab_anchor);
                ...
                if pointer.pointer_type == PointerType::Touch
                    && let Some(id) = pointer.pointer_id
                {
                    let start_scroll = self
                        .estimated_scroll_y(ctx.size().height)
                        .unwrap_or(self.scroll_offset_from_anchor.max(0.0));
                    self.touch_pan = Some((id, local.y, start_scroll));
                }
            }
            PointerEvent::Move(PointerUpdate {
                ...
                if pointer.pointer_type == PointerType::Touch
                    && let (Some(id), Some((active_id, start_y, start_scroll))) =
                        (pointer.pointer_id, self.touch_pan)
                    && id == active_id
                {
                    let viewport_height = ctx.size().height.max(1.0);
                    let finger_delta = start_y - local.y;
                    if let Some(target) =
                        self.touch_scroll_target(start_scroll, finger_delta, viewport_height)
                        && let Some(result) = self.set_estimated_scroll_y(target, viewport_height)
                    {
                        Self::request_estimated_scroll_update(ctx, result);
                    }
                }
            ...
            PointerEvent::Up(PointerButtonEvent { pointer, .. }) => {
                let touch_finished = pointer.pointer_id.is_some_and(|id| {
                    self.touch_pan
            ...
            PointerEvent::Cancel(pointer) => {
                let touch_cancelled = pointer.pointer_id.is_some_and(|id| {
                    self.touch_pan
                        .is_some_and(|(active_id, _, _)| active_id == id)
                });
    ...
    fn post_paint(
        ...
        if let Some((_, thumb, _)) = self.scrollbar_geometry(ctx.size()) {
            let thumb = thumb.to_rounded_rect(theme::SCROLLBAR_RADIUS);
            fill_color(scene, &thumb, theme::SCROLLBAR_COLOR);
            stroke(
                scene,
                &thumb,
...
mod tests {
    ...
    #[test]
    fn fast_explorer_touch_gain_is_bounded() {
        let mut scroll = VirtualScroll::new(0).with_valid_range(0..1_000);
        scroll.mean_item_height = 56.0;
        assert_eq!(scroll.touch_scroll_gain(800.0), Some(2.0));
        assert_eq!(scroll.touch_scroll_target(0.0, 50.0, 800.0), Some(100.0));
        let max_scroll = scroll.estimated_max_scroll_y(800.0).unwrap();
        assert_eq!(scroll.touch_scroll_target(0.0, 720.0, 800.0), Some(max_scroll));
        assert_eq!(
            scroll.touch_scroll_target(max_scroll, -720.0, 800.0),
            Some(0.0)
        );

        let mut short_scroll = VirtualScroll::new(0).with_valid_range(0..9);
        short_scroll.mean_item_height = 100.0;
        assert_eq!(short_scroll.touch_scroll_gain(800.0), Some(1.0));
    }

    #[test]
    fn fast_explorer_scrollbar_reaches_exact_end() {
        let mut scroll = VirtualScroll::new(0).with_valid_range(0..100);
        scroll.mean_item_height = 56.0;
        let viewport = 800.0;
        let max_scroll = scroll.estimated_max_scroll_y(viewport).unwrap();
        assert_eq!(max_scroll, 4_800.0);
        assert!(scroll.set_estimated_scroll_y(max_scroll, viewport));
        assert_eq!(scroll.estimated_scroll_y(viewport), Some(max_scroll));

        let (_, thumb, _) = scroll
            .scrollbar_geometry(Size::new(400.0, viewport))
            .expect("scrollbar should be visible");
        assert!((thumb.y1 + crate::theme::SCROLLBAR_PAD - viewport).abs() < 0.001);
    }
