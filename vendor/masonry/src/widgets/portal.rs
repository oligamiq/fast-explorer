...
use crate::core::{
    ...
    LayoutCtx, NewWidget, NoAction, PaintCtx, PointerButtonEvent, PointerEvent, PointerId,
...
impl<W: Widget + FromDynWidget + ?Sized> Portal<W> {
    ...
    pub fn set_viewport_pos(this: &mut WidgetMut<'_, Self>, position: Point) -> bool {
        ...
        if pos_changed {
            let progress_x = if content_size.width > portal_size.width {
                this.widget.viewport_pos.x / (content_size - portal_size).width
            } else {
                0.0
            };
            ...
            let progress_y = if content_size.height > portal_size.height {
                this.widget.viewport_pos.y / (content_size - portal_size).height
            } else {
                0.0
            };
    ...
    fn on_pointer_event(
        ...
        match event {
            ...
            PointerEvent::Move(PointerUpdate {
                ...
                if let (Some(id), Some((active_id, start, viewport_start))) =
                    ...
                    && id == active_id
                {
                    ...
                    self.set_viewport_pos_raw(
                        portal_size,
                        content_size,
                        viewport_start + (start - current),
                    );
                    let scroll_range = (content_size - portal_size)
                        .clamp(Size::ZERO, Size::new(f64::INFINITY, f64::INFINITY));
                    {
                        let (scrollbar, mut scrollbar_ctx) =
                            ctx.get_raw_mut(&mut self.scrollbar_horizontal);
                        scrollbar.cursor_progress = if scroll_range.width > 0.0 {
                    ...
                    }
                    {
                        ...
                        scrollbar.cursor_progress = if scroll_range.height > 0.0 {
            ...
            PointerEvent::Up(PointerButtonEvent { pointer, .. })
                if pointer.pointer_type == PointerType::Touch =>
            {
                if pointer.pointer_id.is_some_and(|id| {
                    self.touch_pan
            ...
            PointerEvent::Scroll(PointerScrollEvent { delta, .. }) => {
                ...
                let delta = match delta {
                    ScrollDelta::PixelDelta(PhysicalPosition::<f64> { x, y }) => {
                        -Vec2 { x: *x, y: *y }
                    }
