...
impl<const EDITABLE: bool> TextArea<EDITABLE> {
    ...
    pub fn reset_text(this: &mut WidgetMut<'_, Self>, new_text: &str) {
        // A rebuild that only mirrors the already-committed application value must not
        // destroy an active IME composition. `text()` intentionally excludes preedit.
        if this.widget.text().to_string() == new_text {
            return;
        }
        // If the IME is currently composing and the application really changed the
        // committed value, clear the compose before replacing the buffer.
        ...
        let (text, selection, compose) = this.widget.platform_ime_state();
        this.ctx.set_ime_text_state(text, selection, compose);
        this.ctx.request_layout();
    }
    ...
    fn platform_ime_state(&self) -> (String, Range<usize>, Option<Range<usize>>) {
        (
    ...
    fn normalize_ime_range(text: &str, range: &Range<usize>) -> Range<usize> {
        fn boundary_at_or_before(text: &str, index: usize) -> usize {
        ...
        let start = boundary_at_or_before(text, range.start);
        let end = boundary_at_or_before(text, range.end);
    ...
    fn on_text_event(
        ...
        match event {
            ...
            TextEvent::Ime(e) => {
                ...
                // Whether committed text changed. Preedit only updates the editor's
                // composition overlay; publishing it to application state would cause
                // the subsequent rebuild to reset/clear the active IME composition.
                let mut committed_edit = false;
                match e {
                    Ime::Disabled => {
                    ...
                    Ime::BufferState {
                        ...
                        let old_committed = self.text().to_string();
                        let has_line_break = text.chars().any(|ch| matches!(ch, '\n' | '\r'));

                        if self.insert_newline == InsertNewline::Never && has_line_break {
                            let normalized = text
                                .replace("\r\n", "\n")
                                .replace('\r', "\n")
                                .split('\n')
                            ...
                            ctx.submit_action::<Self::Action>(TextAction::Entered(normalized));
                        } else {
                            let selection = Self::normalize_ime_range(text, selection);
                            let compose = compose
                                .as_ref()
                                .map(|range| Self::normalize_ime_range(text, range))
                            ...
                            if let Some(compose) = compose {
                                driver.set_compose_byte_range(compose.start, compose.end);
                            }
                            drop(driver);

                            committed_edit = old_committed != self.text().to_string();
                        }
                    }
                    Ime::Commit(text) if self.insert_newline == InsertNewline::Never => {
                        if let Some((newline_index, _)) = text
                            .char_indices()
                            .find(|(_, ch)| matches!(ch, '\n' | '\r'))
                        {
                            let before_newline = &text[..newline_index];
                            if !before_newline.is_empty() {
                                self.editor
                                    .driver(fctx, lctx)
                                    .insert_or_replace_selection(before_newline);
                            }
                            ctx.submit_action::<Self::Action>(TextAction::Entered(
                                self.text().to_string(),
                            ));
            ...
            TextEvent::ClipboardPaste(text) => {
                if EDITABLE {
                    let (fctx, lctx) = ctx.text_contexts();
                    let text = if self.insert_newline == InsertNewline::Never {
...
mod tests {
    ...
    #[test]
    fn ime_newline_commit_submits_single_line_input() {
        let area = NewWidget::new(
            TextArea::new_editable("hello world").with_insert_newline(InsertNewline::Never),
        );
        let mut harness = TestHarness::create(default_property_set(), area);
        let text_id = harness.root_id();
        harness.focus_on(Some(text_id));
        harness.process_text_event(TextEvent::Ime(Ime::Commit("\n".to_string())));

        assert_eq!(harness.root_widget().text().to_string(), "hello world");
