...
fn android_utf16_offset_to_utf8(text: &str, utf16_offset: usize) -> usize {
...
fn android_utf16_range_to_utf8(text: &str, range: std::ops::Range<usize>) -> std::ops::Range<usize> {
    let start = android_utf16_offset_to_utf8(text, range.start);
    let end = android_utf16_offset_to_utf8(text, range.end);
    start.min(end)..start.max(end)
}

fn utf8_byte_offset_to_android_utf16(text: &str, byte_offset: usize) -> usize {
        ...
        F: FnMut(event::Event<T>, &RootAEL),
    {
        ...
        match event {
            ...
            InputEvent::TextEvent(state) => {
                let event = event::Event::WindowEvent {
                    window_id: window::WindowId(WindowId),
                    event: event::WindowEvent::Ime(event::Ime::BufferState {
                        text: state.text.clone(),
                        selection: android_utf16_range_to_utf8(
                            &state.text,
                            state.selection.start..state.selection.end,
                        ),
                        compose: state.compose_region.as_ref().map(|span| {
                            android_utf16_range_to_utf8(&state.text, span.start..span.end)
                        }),
                    }),
                };
                callback(event, self.window_target());
...
impl Window {
    ...
    pub fn set_ime_buffer_state(
        ...
        let selection = TextSpan {
            start: utf8_byte_offset_to_android_utf16(&text, selection.start),
            end: utf8_byte_offset_to_android_utf16(&text, selection.end),
        };
        let compose_region = compose.map(|range| TextSpan {
            start: utf8_byte_offset_to_android_utf16(&text, range.start),
            end: utf8_byte_offset_to_android_utf16(&text, range.end),
        });
